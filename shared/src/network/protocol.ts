...
export interface WorldSnapshot { tick: number; players: NetworkPlayer[]; combat?: CombatWorld }
export interface Welcome { id: string; token: string; epoch: string; snapshot: WorldSnapshot }
export interface JoinAuth { version: number; name: string; token?: string }
export type NetworkError = 'INCOMPATIBLE' | 'INVALID_NAME' | 'FULL' | 'EXPIRED' | 'IN_USE' | 'RATE_LIMIT' | 'INVALID_INPUT';
